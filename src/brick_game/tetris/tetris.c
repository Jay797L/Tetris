...
int full_field(){
  ...
  int res = 0;
  if(block.x < 0 || block.x + block.rows > LENGTH || 
     block.y < 0 || block.y + block.columns > WIDTH) {
    res = 1;
  }
  ...
  for(int i = 0; i < LENGTH; i++){
    for(int j = 0; j < WIDTH; j++){
      ...
      if(is_now_x < block.rows && is_now_x >= 0 && is_now_y < block.columns && is_now_y >= 0) state->screen.field[i][j] += block.matrix[is_now_x][is_now_y];
...
void remove_strings(){
  ...
  for(int i = LENGTH - 1; i >= 0; i--){
    while((i-shift > 0) && (state->screen.field[i-shift][0] == 2)) shift+= 1;
    for (int j = 0; j < WIDTH; j++){
      if(i-shift >= 0){
